...
plot (t,m);
xlabel ('Time(s)');
ylabel ('Amplitude');                %message signal
title ('Message Signal');
grid on
c = a.*sin (2*pi*fc*t);
subplot (3,1,2);
plot (t,c);                         %carrier signal
xlabel ('Time(s)');
ylabel ('Amplitude');
...
plot (t,s);
xlabel ('Time(s)');                    
ylabel ('Amplitude');                  %AM signal
title ('Modulated Signal');
grid on
M = fftshift(fft(m));
C = fftshift(fft(c));
S = fftshift(fft(s));                  %frequency spectrums
f = -Fs/2:1:Fs/2-1;
figure
subplot(4,1,1)
plot(f,abs(M)/Fs);
...
subplot(4,1,4)
plot(f,abs(R_flt)/Fs);
title('Freq. Spectrum of Demodulated Signal')     %Demodualted signal
xlabel('Frequency (Hz)')
ylabel('Magnitude')
grid on
figure
